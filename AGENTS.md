...
Do not improvise a release. `package.json` is the only version you edit, and a bump without notes under `## [Unreleased]` in `CHANGELOG.md` fails CI. The manifests, the dated changelog heading and the README release block are generated by `npm run version:sync`; the tag, the npm package and the GitHub release are created by the **Release** workflow, which needs two merges. The process, the repository settings it depends on and every failure mode are in [docs/releasing.md](docs/releasing.md).
