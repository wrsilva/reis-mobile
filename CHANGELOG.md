...
- 24 skills from [android/skills](https://github.com/android/skills) (Apache-2.0), credited in `THIRD_PARTY_NOTICES.md`: `android-agp-9-upgrade`, `android-r8-analyzer`, `android-intent-security`, `android-profiler` and `android-play-policy-insights` are routed automatically, and `testing-setup` became part of `mobile-test`; the others (Compose, navigation, Play, camera, media, identity, on-device AI, Wear, TV, XR and the Android CLI) are triggered by their description.
