...
| RELEASE-02 | Criteria 3–6 | Command identity-field contract, verdict unit/CLI tests and read-only integration | verified |
| RELEASE-03 | Criterion 7 | Discoverability/docs, package smoke check, representative audit and final full suite | verified |
...
The final review also requested official sources beside the example's claims about Android/iOS build-number scope and staged/phased rollout recovery. Added those primary-source links to the example and shared reference.

Final verification: `npm run check` passed validation (5 stacks, 21 agents, 14 skills) and all 222 tests with zero failures or skips. A scratch-worktree mutation removed `store/track` from the release identity instructions; the focused command-contract test failed on the missing field as expected, then the scratch tree was removed and the repository status remained unchanged. The exact AC-to-test evidence and package/sample checks are recorded in `validation.md`.
