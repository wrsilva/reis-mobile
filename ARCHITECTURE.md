...
`package.json` is the only editable version source. `scripts/versions.mjs --sync` updates the Claude Code manifests, promotes authored `[Unreleased]` notes to the new dated changelog section, and derives the README's latest-release block from that section. Historical release entries and roadmap targets are not rewritten. Tests reject drift or a new version without release notes. On a `main` push, the Release workflow synchronizes and tests the package. If that changes release files, it pushes a namespaced branch and opens a pull request instead of pushing directly to protected `main`; tagging and publishing wait for that PR to pass the required checks and merge. The next `main` run creates the `vX.Y.Z` tag, publishes to npm and creates the GitHub release. Existing tags skip a main-branch release; tag pushes and manual tag retries preserve the idempotent publish path. Creating the PR with the default `GITHUB_TOKEN` requires the repository's **Allow GitHub Actions to create and approve pull requests** setting, and its checks may require manual approval. An optional `RELEASE_PR_TOKEN` with repository contents and pull-request write access can create the PR and trigger checks without that approval gate. The workflow stops before tagging if it cannot open the PR or publish to npm.
